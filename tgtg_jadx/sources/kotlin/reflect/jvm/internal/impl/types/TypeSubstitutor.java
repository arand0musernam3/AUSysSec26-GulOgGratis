...
public class TypeSubstitutor {
    ...
    */
    ...
    private static void assertRecursionDepth(int i11, TypeProjection typeProjection, TypeSubstitution typeSubstitution) {
        ...
        d0.t("Recursion too deep. Most likely infinite loop while substituting ", safeToString(typeProjection), "; substitution: ", safeToString(typeSubstitution));
    }
    ...
    private static String safeToString(Object obj) {
        try {
            ...
            return f.l("[Exception while computing toString(): ", "]", th2);
        }
    }
    ...
    private TypeProjection substituteCompoundType(TypeProjection typeProjection, int i11) throws SubstitutionException {
        ...
        if (type.getConstructor().mo977getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return typeProjection;
        }
        ...
        KotlinType kotlinTypeReplace = TypeSubstitutionKt.replace(type, substituteTypeArguments(type.getConstructor().getParameters(), type.getArguments(), i11), this.substitution.filterAnnotations(type.getAnnotations()));
    ...
    private List<TypeProjection> substituteTypeArguments(List<TypeParameterDescriptor> list, List<TypeProjection> list2, int i11) throws SubstitutionException {
        ...
        for (int i12 = 0; i12 < list.size(); i12++) {
            ...
            TypeProjection typeProjectionUnsafeSubstitute = unsafeSubstitute(typeProjection, typeParameterDescriptor, i11 + 1);
            int i13 = AnonymousClass2.$SwitchMap$org$jetbrains$kotlin$types$TypeSubstitutor$VarianceConflictType[conflictType(typeParameterDescriptor.getVariance(), typeProjectionUnsafeSubstitute.getProjectionKind()).ordinal()];
            if (i13 == 1 || i13 == 2) {
    ...
    private TypeProjection unsafeSubstitute(@NotNull TypeProjection typeProjection, @Nullable TypeParameterDescriptor typeParameterDescriptor, int i11) throws SubstitutionException {
        ...
        if (!typeProjection.isStarProjection()) {
            ...
            if (type instanceof TypeWithEnhancement) {
                ...
                TypeProjection typeProjectionUnsafeSubstitute = unsafeSubstitute(new TypeProjectionImpl(typeProjection.getProjectionKind(), origin), typeParameterDescriptor, i11 + 1);
            ...
            if (!DynamicTypesKt.isDynamic(type) && !(type.unwrap() instanceof RawType)) {
                TypeProjection typeProjectionMo982get = this.substitution.mo982get(type);
                TypeProjection typeProjectionProjectedTypeForConflictedTypeWithUnsafeVariance = typeProjectionMo982get != null ? projectedTypeForConflictedTypeWithUnsafeVariance(type, typeProjectionMo982get, typeParameterDescriptor, typeProjection) : null;
                Variance projectionKind = typeProjection.getProjectionKind();
                if (typeProjectionProjectedTypeForConflictedTypeWithUnsafeVariance == null && FlexibleTypesKt.isFlexible(type) && !TypeCapabilitiesKt.isCustomTypeParameter(type)) {
                    ...
                    int i12 = i11 + 1;
                    TypeProjection typeProjectionUnsafeSubstitute2 = unsafeSubstitute(new TypeProjectionImpl(projectionKind, flexibleTypeAsFlexibleType.getLowerBound()), typeParameterDescriptor, i12);
                    TypeProjection typeProjectionUnsafeSubstitute3 = unsafeSubstitute(new TypeProjectionImpl(projectionKind, flexibleTypeAsFlexibleType.getUpperBound()), typeParameterDescriptor, i12);
                    ...
                    if (typeProjectionProjectedTypeForConflictedTypeWithUnsafeVariance != null) {
                        ...
                        if (!CapturedTypeConstructorKt.isCaptured(type)) {
                            int i13 = AnonymousClass2.$SwitchMap$org$jetbrains$kotlin$types$TypeSubstitutor$VarianceConflictType[varianceConflictTypeConflictType.ordinal()];
                            if (i13 == 1) {
    ...
    public KotlinType safeSubstitute(@NotNull KotlinType kotlinType, @NotNull Variance variance) {
        ...
        try {
            ...
            ErrorType errorTypeCreateErrorType = ErrorUtils.createErrorType(ErrorTypeKind.UNABLE_TO_SUBSTITUTE_TYPE, e5.getMessage());
