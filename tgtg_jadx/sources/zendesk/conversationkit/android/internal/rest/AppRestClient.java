package zendesk.conversationkit.android.internal.rest;

import e0.f;
import kotlin.Metadata;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import x70.c;
import zendesk.conversationkit.android.internal.rest.model.AppUserRequestDto;
import zendesk.conversationkit.android.internal.rest.model.AppUserResponseDto;
import zendesk.conversationkit.android.internal.rest.user.model.LoginRequestBody;
import zendesk.faye.internal.Bayeux;

/* JADX INFO: compiled from: r8-map-id-bad8e3e1b75e2d3fd72c92f1b960553adb24fef824cc9e98cee450b85845d4c2 */
/* JADX INFO: loaded from: classes3.dex */
@Metadata(d1 = {"\u0000,\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0004\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0002\b\u0006\b\u0000\u0018\u00002\u00020\u0001B\u0017\u0012\u0006\u0010\u0003\u001a\u00020\u0002\u0012\u0006\u0010\u0005\u001a\u00020\u0004¢\u0006\u0004\b\u0006\u0010\u0007J \u0010\f\u001a\u00020\u000b2\u0006\u0010\b\u001a\u00020\u00022\u0006\u0010\n\u001a\u00020\tH\u0086@¢\u0006\u0004\b\f\u0010\rJ \u0010\u0011\u001a\u00020\u000b2\u0006\u0010\u000e\u001a\u00020\u00022\u0006\u0010\u0010\u001a\u00020\u000fH\u0086@¢\u0006\u0004\b\u0011\u0010\u0012R\u0014\u0010\u0003\u001a\u00020\u00028\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\u0003\u0010\u0013R\u0014\u0010\u0005\u001a\u00020\u00048\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\u0005\u0010\u0014¨\u0006\u0015"}, d2 = {"Lzendesk/conversationkit/android/internal/rest/AppRestClient;", "", "", "appId", "Lzendesk/conversationkit/android/internal/rest/SunshineConversationsApi;", "sunshineConversationsApi", "<init>", "(Ljava/lang/String;Lzendesk/conversationkit/android/internal/rest/SunshineConversationsApi;)V", Bayeux.KEY_CLIENT_ID, "Lzendesk/conversationkit/android/internal/rest/model/AppUserRequestDto;", "appUserRequestDto", "Lzendesk/conversationkit/android/internal/rest/model/AppUserResponseDto;", "createAppUser", "(Ljava/lang/String;Lzendesk/conversationkit/android/internal/rest/model/AppUserRequestDto;Lx70/c;)Ljava/lang/Object;", "jwt", "Lzendesk/conversationkit/android/internal/rest/user/model/LoginRequestBody;", "loginRequestBody", "loginAppUser", "(Ljava/lang/String;Lzendesk/conversationkit/android/internal/rest/user/model/LoginRequestBody;Lx70/c;)Ljava/lang/Object;", "Ljava/lang/String;", "Lzendesk/conversationkit/android/internal/rest/SunshineConversationsApi;", "zendesk.conversationkit_conversationkit-android"}, k = 1, mv = {2, 1, 0}, xi = 48)
public final class AppRestClient {

    @NotNull
    private final String appId;

    @NotNull
    private final SunshineConversationsApi sunshineConversationsApi;

    public AppRestClient(@NotNull String str, @NotNull SunshineConversationsApi sunshineConversationsApi) {
        str.getClass();
        sunshineConversationsApi.getClass();
        this.appId = str;
        this.sunshineConversationsApi = sunshineConversationsApi;
    }

    @Nullable
    public final Object createAppUser(@NotNull String str, @NotNull AppUserRequestDto appUserRequestDto, @NotNull c<? super AppUserResponseDto> cVar) {
        return this.sunshineConversationsApi.createAppUser(this.appId, str, appUserRequestDto, cVar);
    }

    @Nullable
    public final Object loginAppUser(@NotNull String str, @NotNull LoginRequestBody loginRequestBody, @NotNull c<? super AppUserResponseDto> cVar) {
        return this.sunshineConversationsApi.loginAppUser(this.appId, f.k("Bearer ", str), loginRequestBody, cVar);
    }
}
