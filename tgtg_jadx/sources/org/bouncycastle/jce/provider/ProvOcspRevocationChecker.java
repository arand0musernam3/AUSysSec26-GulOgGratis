...
class ProvOcspRevocationChecker implements PKIXCertRevocationChecker {
    ...
    private CertID createCertID(AlgorithmIdentifier algorithmIdentifier, Certificate certificate, ASN1Integer aSN1Integer) throws CertPathValidatorException {
        try {
            ...
            throw new CertPathValidatorException(i.l("problem creating ID: ", e5), e5);
        }
    }
    ...
    private Certificate extractCert() throws CertPathValidatorException {
        try {
            ...
            throw new CertPathValidatorException(k.j(e5, new StringBuilder("cannot process signing cert: ")), e5, this.parameters.getCertPath(), this.parameters.getIndex());
        }
    }
    ...
    private static String getDigestName(ASN1ObjectIdentifier aSN1ObjectIdentifier) {
        ...
        return (iIndexOf <= 0 || digestName.startsWith("SHA3")) ? digestName : digestName.substring(0, iIndexOf).concat(digestName.substring(iIndexOf + 1));
    }
    ...
    public static URI getOcspResponderURI(X509Certificate x509Certificate) {
        ...
        for (int i11 = 0; i11 != accessDescriptions.length; i11++) {
            AccessDescription accessDescription = accessDescriptions[i11];
    ...
    private static String getSignatureName(AlgorithmIdentifier algorithmIdentifier) {
        ...
        if (parameters != null && !DERNull.INSTANCE.equals(parameters) && algorithmIdentifier.getAlgorithm().equals((ASN1Primitive) PKCSObjectIdentifiers.id_RSASSA_PSS)) {
            return k.p(new StringBuilder(), getDigestName(RSASSAPSSparams.getInstance(parameters).getHashAlgorithm().getAlgorithm()), "WITHRSAANDMGF1");
        }
        Map map = oids;
    ...
    public static boolean validatedOcspResponse(BasicOCSPResponse basicOCSPResponse, PKIXCertRevocationCheckerParameters pKIXCertRevocationCheckerParameters, byte[] bArr, X509Certificate x509Certificate, JcaJceHelper jcaJceHelper) throws CertPathValidatorException {
        try {
            ...
            if (!signatureCreateSignature.verify(basicOCSPResponse.getSignature().getOctets())) {
                return false;
            }
            ...
            throw new CertPathValidatorException(s.g(e5, new StringBuilder("OCSP response failure: ")), e5, pKIXCertRevocationCheckerParameters.getCertPath(), pKIXCertRevocationCheckerParameters.getIndex());
    ...
    public void check(java.security.cert.Certificate certificate) throws CertPathValidatorException {
        ...
        if (ocspResponder == null) {
            if (this.ocspURL != null) {
                try {
                    ...
                    throw new CertPathValidatorException("configuration error: " + e5.getMessage(), e5, this.parameters.getCertPath(), this.parameters.getIndex());
                }
            } else {
        ...
        if (ocspResponses.get(x509Certificate) != null || uri == null) {
            ...
            for (int i11 = 0; i11 != ocspExtensions.size(); i11++) {
                java.security.cert.Extension extension = ocspExtensions.get(i11);
        ...
        if (responseBytes.getResponseType().equals((ASN1Primitive) OCSPObjectIdentifiers.id_pkix_ocsp_basic)) {
            try {
                ...
                if (!z11 && !validatedOcspResponse(basicOCSPResponse, this.parameters, bArr, this.parent.getOcspResponderCert(), this.helper)) {
                    return;
                }
                ...
                for (int i12 = 0; i12 != responses.size(); i12++) {
                    SingleResponse singleResponse = SingleResponse.getInstance(responses.getObjectAt(i12));
                    if (aSN1Integer.equals((ASN1Primitive) singleResponse.getCertID().getSerialNumber())) {
                        ...
                        if (certIDCreateCertID == null || !isEqualAlgId(certIDCreateCertID.getHashAlgorithm(), singleResponse.getCertID().getHashAlgorithm())) {
