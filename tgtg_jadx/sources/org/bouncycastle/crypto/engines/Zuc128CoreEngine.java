...
public class Zuc128CoreEngine implements StreamCipher, Memoable {
    ...
    private void LFSRWithInitialisationMode(int i11) {
        int i12 = this.LFSR[0];
        int iAddM = AddM(AddM(AddM(AddM(AddM(AddM(i12, MulByPow2(i12, 8)), MulByPow2(this.LFSR[4], 20)), MulByPow2(this.LFSR[10], 21)), MulByPow2(this.LFSR[13], 17)), MulByPow2(this.LFSR[15], 15)), i11);
        int[] iArr = this.LFSR;
        iArr[0] = iArr[1];
        iArr[1] = iArr[2];
        iArr[2] = iArr[3];
    ...
    private void LFSRWithWorkMode() {
        int i11 = this.LFSR[0];
        int iAddM = AddM(AddM(AddM(AddM(AddM(i11, MulByPow2(i11, 8)), MulByPow2(this.LFSR[4], 20)), MulByPow2(this.LFSR[10], 21)), MulByPow2(this.LFSR[13], 17)), MulByPow2(this.LFSR[15], 15));
        int[] iArr = this.LFSR;
        iArr[0] = iArr[1];
        iArr[1] = iArr[2];
        iArr[2] = iArr[3];
    ...
    private static int MAKEU31(byte b8, short s7, byte b11) {
        return ((b8 & 255) << 23) | ((s7 & HPKE.aead_EXPORT_ONLY) << 8) | (b11 & 255);
    }

    private static int MAKEU32(byte b8, byte b11, byte b12, byte b13) {
        return ((b8 & 255) << 24) | ((b11 & 255) << 16) | ((b12 & 255) << 8) | (b13 & 255);
    }
    ...
    public int F() {
        ...
        int i15 = i12 + iArr[1];
        int i16 = iArr[2] ^ i13;
        int iL1 = L1((i15 << 16) | (i16 >>> 16));
        int iL2 = L2((i16 << 16) | (i15 >>> 16));
        int[] iArr3 = this.F;
        byte[] bArr = S0;
        byte b8 = bArr[iL1 >>> 24];
        byte[] bArr2 = S1;
        iArr3[0] = MAKEU32(b8, bArr2[(iL1 >>> 16) & 255], bArr[(iL1 >>> 8) & 255], bArr2[iL1 & 255]);
    ...
    public void init(boolean z11, CipherParameters cipherParameters) {
        ...
        setKeyAndIV(key, iv2);
        CryptoServicesRegistrar.checkConstraints(new DefaultServiceProperties(getAlgorithmName(), key.length * 8, cipherParameters, z11 ? CryptoServicePurpose.ENCRYPTION : CryptoServicePurpose.DECRYPTION));
    ...
    public int makeKeyStreamWord() {
        ...
        if (i11 >= getMaxIterations()) {
            h2.b("Too much data processed by singleKey/IV");
            return 0;
        }
        BitReorganization();
        int iF = F() ^ this.BRC[3];
    ...
    public void setKeyAndIV(int[] iArr, byte[] bArr, byte[] bArr2) {
        if (bArr == null || bArr.length != 16) {
            a.f("A key of 16 bytes is needed");
            return;
        }
        if (bArr2 == null || bArr2.length != 16) {
            a.f("An IV of 16 bytes is needed");
            return;
        }
        int[] iArr2 = this.LFSR;
        byte b8 = bArr[0];
        short[] sArr = EK_d;
        iArr2[0] = MAKEU31(b8, sArr[0], bArr2[0]);
