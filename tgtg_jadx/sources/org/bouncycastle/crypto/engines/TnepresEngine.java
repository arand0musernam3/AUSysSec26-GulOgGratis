...
public final class TnepresEngine extends SerpentEngineBase {
    ...
    public void decryptBlock(byte[] bArr, int i11, byte[] bArr2, int i12) {
        int[] iArr = {iBigEndianToInt, this.wKey[129] ^ Pack.bigEndianToInt(bArr, i11 + 8), this.wKey[130] ^ Pack.bigEndianToInt(bArr, i11 + 4), this.wKey[131] ^ Pack.bigEndianToInt(bArr, i11)};
        int iBigEndianToInt = this.wKey[128] ^ Pack.bigEndianToInt(bArr, i11 + 12);
        ib7(iArr, iBigEndianToInt, iArr[1], iArr[2], iArr[3]);
        int i13 = iArr[0];
        int[] iArr2 = this.wKey;
        iArr[0] = i13 ^ iArr2[124];
        ...
        inverseLT(iArr);
        ib0(iArr, iArr[0], iArr[1], iArr[2], iArr[3]);
        Pack.intToBigEndian(iArr[3] ^ this.wKey[3], bArr2, i12);
        Pack.intToBigEndian(iArr[2] ^ this.wKey[2], bArr2, i12 + 4);
        Pack.intToBigEndian(iArr[1] ^ this.wKey[1], bArr2, i12 + 8);
        Pack.intToBigEndian(this.wKey[0] ^ iArr[0], bArr2, i12 + 12);
    }

    @Override // org.bouncycastle.crypto.engines.SerpentEngineBase
    public void encryptBlock(byte[] bArr, int i11, byte[] bArr2, int i12) {
        int[] iArr = {iBigEndianToInt, Pack.bigEndianToInt(bArr, i11 + 8), Pack.bigEndianToInt(bArr, i11 + 4), Pack.bigEndianToInt(bArr, i11)};
        int iBigEndianToInt = Pack.bigEndianToInt(bArr, i11 + 12);
        int[] iArr2 = this.wKey;
        ...
        Pack.intToBigEndian(this.wKey[131] ^ iArr[3], bArr2, i12);
        Pack.intToBigEndian(this.wKey[130] ^ iArr[2], bArr2, i12 + 4);
        Pack.intToBigEndian(this.wKey[129] ^ iArr[1], bArr2, i12 + 8);
        Pack.intToBigEndian(this.wKey[128] ^ iArr[0], bArr2, i12 + 12);
    }
    ...
    public int[] makeWorkingKey(byte[] bArr) throws IllegalArgumentException {
        ...
        int i11 = 0;
        while (length > 0) {
            iArr[i11] = Pack.bigEndianToInt(bArr, length);
            length -= 4;
            i11++;
        }
        if (length != 0) {
            a.f("key must be a multiple of 4 bytes");
            return null;
        }
        int i12 = i11 + 1;
        iArr[i11] = Pack.bigEndianToInt(bArr, 0);
        if (i12 < 8) {
            iArr[i12] = 1;
        }
        int[] iArr2 = new int[AppConstants.RESULT_CODE_FLASH_SALES_SURVEY];
        for (int i13 = 8; i13 < 16; i13++) {
            int i14 = i13 - 8;
            iArr[i13] = SerpentEngineBase.rotateLeft(((-1640531527) ^ (((iArr[i14] ^ iArr[i13 - 5]) ^ iArr[i13 - 3]) ^ iArr[i13 - 1])) ^ i14, 11);
        }
        System.arraycopy(iArr, 8, iArr2, 0, 8);
        for (int i15 = 8; i15 < 132; i15++) {
            iArr2[i15] = SerpentEngineBase.rotateLeft(((((iArr2[i15 - 8] ^ iArr2[i15 - 5]) ^ iArr2[i15 - 3]) ^ iArr2[i15 - 1]) ^ (-1640531527)) ^ i15, 11);
        }
        int[] iArr3 = new int[4];
