...
public class SubjectPublicKeyInfoFactory {
    private static Set cryptoProOids = null;
    ...
    public static SubjectPublicKeyInfo createSubjectPublicKeyInfo(AsymmetricKeyParameter asymmetricKeyParameter) throws IOException {
        ...
        if (!(asymmetricKeyParameter instanceof ECPublicKeyParameters)) {
            ...
            if (asymmetricKeyParameter instanceof HSSPublicKeyParameters) {
                ...
                byte[] bArrBuild = Composer.compose().u32str(hSSPublicKeyParameters.getL()).bytes(hSSPublicKeyParameters.getLMSPublicKey()).build();
                return new SubjectPublicKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.id_alg_hss_lms_hashsig), Arrays.concatenate(new byte[]{4, (byte) bArrBuild.length}, bArrBuild));
            }
            if (asymmetricKeyParameter instanceof LMSPublicKeyParameters) {
                byte[] bArrBuild2 = Composer.compose().u32str(1).bytes((LMSPublicKeyParameters) asymmetricKeyParameter).build();
                return new SubjectPublicKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.id_alg_hss_lms_hashsig), Arrays.concatenate(new byte[]{4, (byte) bArrBuild2.length}, bArrBuild2));
            }
        ...
        if (parameters2 == null) {
            ...
            if (parameters2 instanceof ECGOST3410Parameters) {
                ...
                int i13 = i12 / 2;
                extractBytes(bArr, i13, 0, bigInteger);
                extractBytes(bArr, i13, i11, bigInteger2);
                try {
    ...
    private static void extractBytes(byte[] bArr, int i11, int i12, BigInteger bigInteger) {
