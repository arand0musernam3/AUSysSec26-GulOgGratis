...
public class OpenSSHPrivateKeyUtil {
    ...
    public static byte[] encodePrivateKey(AsymmetricKeyParameter asymmetricKeyParameter) throws IOException {
        ...
        if (asymmetricKeyParameter instanceof DSAPrivateKeyParameters) {
            ...
            try {
                ...
                h2.b(k.j(e5, new StringBuilder("unable to encode DSAPrivateKeyParameters ")));
                return null;
            }
        }
        if (!(asymmetricKeyParameter instanceof Ed25519PrivateKeyParameters)) {
            i1.j("unable to convert ", asymmetricKeyParameter.getClass().getName(), " to openssh private key");
            return null;
        }
    ...
    public static AsymmetricKeyParameter parsePrivateKeyBlob(byte[] bArr) {
                    ...
                    if (sSHBuffer.hasRemaining()) {
                        ...
                        if (sSHBuffer2.readU32() == sSHBuffer2.readU32()) {
                            ...
                            if ("ssh-ed25519".equals(string)) {
                                ...
                                if (block.length != 64) {
                                    h2.b("private key value of wrong length");
                                    return null;
                                }
                                ...
                                if (byName != null) {
                                    ...
                                    if (byOID != null) {
                                        ...
                                        strConcat = s.l("Curve not found for: ", byName);
                                    }
                                } else {
                                    strConcat = "OID not found for: ".concat(string);
                                }
                            ...
                            if (sSHBuffer2.hasRemaining()) {
                                str = "private key block has trailing data";
                            }
                        } else {
                            strConcat = "private key check values are not the same";
                        }
                    }
                    a.f(str);
                    return null;
                }
                strConcat = "multiple keys not supported";
            } else {
                strConcat = "encrypted keys not supported";
            }
            h2.b(strConcat);
            return null;
        }
        ...
        if (aSN1Sequence.size() == 6) {
            ...
            rSAPrivateCrtKeyParameters = new ECPrivateKeyParameters(eCPrivateKey.getKey(), zIsNamedCurve ? ECNamedDomainParameters.lookup(ASN1ObjectIdentifier.getInstance(parameters)) : new ECDomainParameters(X9ECParameters.getInstance(parameters)));
        }
