...
public class SRP6Util {
    ...
    private static byte[] getPadded(BigInteger bigInteger, int i11) {
        byte[] bArrAsUnsignedByteArray = BigIntegers.asUnsignedByteArray(bigInteger);
        if (bArrAsUnsignedByteArray.length >= i11) {
            return bArrAsUnsignedByteArray;
        }
        byte[] bArr = new byte[i11];
        System.arraycopy(bArrAsUnsignedByteArray, 0, bArr, i11 - bArrAsUnsignedByteArray.length, bArrAsUnsignedByteArray.length);
        return bArr;
    }
