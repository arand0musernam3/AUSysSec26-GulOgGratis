...
public class HPKE {
    ...
    public HPKE(byte b8, short s7, short s8, short s11) {
        this.mode = b8;
        this.kemId = s7;
        this.kdfId = s8;
        this.aeadId = s11;
        this.hkdf = new HKDF(s8);
        DHKEM dhkem = new DHKEM(s7);
        this.kem = dhkem;
        this.Nk = s11 == 1 ? (short) 16 : (short) 32;
        this.encSize = dhkem.getEncryptionSize();
    }

    private void VerifyPSKInputs(byte b8, byte[] bArr, byte[] bArr2) {
        String str;
        ...
        } else if (!zAreEqual && b8 % 2 == 0) {
            str = "PSK input provided when not needed";
        } else if (!zAreEqual || b8 % 2 != 1) {
            return;
        } else {
    ...
    private HPKEContext keySchedule(byte b8, byte[] bArr, byte[] bArr2, byte[] bArr3, byte[] bArr4) {
        VerifyPSKInputs(b8, bArr3, bArr4);
        ...
        byte[] bArrConcatenate2 = Arrays.concatenate(new byte[]{b8}, this.hkdf.LabeledExtract(null, bArrConcatenate, "psk_id_hash", bArr4), this.hkdf.LabeledExtract(null, bArrConcatenate, "info_hash", bArr2));
    ...
    public HPKEContextWithEncapsulation SetupPSKS(AsymmetricKeyParameter asymmetricKeyParameter, byte[] bArr, byte[] bArr2, byte[] bArr3) {
        byte[][] bArrEncap = this.kem.Encap(asymmetricKeyParameter);
    ...
    public byte[] receiveExport(byte[] bArr, AsymmetricCipherKeyPair asymmetricCipherKeyPair, byte[] bArr2, byte[] bArr3, int i11, byte[] bArr4, byte[] bArr5, AsymmetricKeyParameter asymmetricKeyParameter) {
        HPKEContext hPKEContext;
        byte b8 = this.mode;
        if (b8 == 0) {
            hPKEContext = setupBaseR(bArr, asymmetricCipherKeyPair, bArr2);
        ...
        return hPKEContext.export(bArr3, i11);
    }
    ...
    public byte[][] sendExport(AsymmetricKeyParameter asymmetricKeyParameter, byte[] bArr, byte[] bArr2, int i11, byte[] bArr3, byte[] bArr4, AsymmetricCipherKeyPair asymmetricCipherKeyPair) {
        HPKEContextWithEncapsulation hPKEContextWithEncapsulationSetupPSKS;
        byte b8 = this.mode;
        if (b8 == 0) {
            hPKEContextWithEncapsulationSetupPSKS = setupBaseS(asymmetricKeyParameter, bArr);
        ...
        return new byte[][]{hPKEContextWithEncapsulationSetupPSKS.encapsulation, hPKEContextWithEncapsulationSetupPSKS.export(bArr2, i11)};
    }
    ...
    public HPKEContextWithEncapsulation setupAuthPSKS(AsymmetricKeyParameter asymmetricKeyParameter, byte[] bArr, byte[] bArr2, byte[] bArr3, AsymmetricCipherKeyPair asymmetricCipherKeyPair) {
        byte[][] bArrAuthEncap = this.kem.AuthEncap(asymmetricKeyParameter, asymmetricCipherKeyPair);
    ...
    public HPKEContextWithEncapsulation setupBaseS(AsymmetricKeyParameter asymmetricKeyParameter, byte[] bArr, AsymmetricCipherKeyPair asymmetricCipherKeyPair) {
        byte[][] bArrEncap = this.kem.Encap(asymmetricKeyParameter, asymmetricCipherKeyPair);
        return new HPKEContextWithEncapsulation(keySchedule((byte) 0, bArrEncap[0], bArr, this.default_psk, this.default_psk_id), bArrEncap[1]);
    }

    public HPKE(byte b8, short s7, short s8, short s11, KEM kem, int i11) {
        this.mode = b8;
        this.kemId = s7;
        this.kdfId = s8;
        this.aeadId = s11;
