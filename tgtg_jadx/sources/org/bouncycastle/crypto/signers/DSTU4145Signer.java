...
public class DSTU4145Signer implements DSAExt {
    ...
    public BigInteger[] generateSignature(byte[] bArr) {
        ...
        while (true) {
            BigInteger bigIntegerGenerateRandomInteger = generateRandomInteger(n11, this.random);
            ...
            if (!affineXCoord.isZero()) {
                BigInteger bigIntegerFieldElement2Integer = fieldElement2Integer(n11, eCFieldElementHash2FieldElement.multiply(affineXCoord));
                if (bigIntegerFieldElement2Integer.signum() != 0) {
                    BigInteger bigIntegerMod = bigIntegerFieldElement2Integer.multiply(d3).add(bigIntegerGenerateRandomInteger).mod(n11);
    ...
    public void init(boolean z11, CipherParameters cipherParameters) {
        ...
        CryptoServicesRegistrar.checkConstraints(Utils.getDefaultProperties("DSTU4145", this.key, z11));
    }
    ...
    public boolean verifySignature(byte[] bArr, BigInteger bigInteger, BigInteger bigInteger2) {
        if (bigInteger.signum() > 0 && bigInteger2.signum() > 0) {
            ...
            if (bigInteger.compareTo(n11) < 0 && bigInteger2.compareTo(n11) < 0) {
                ...
                if (!eCPointNormalize.isInfinity() && fieldElement2Integer(n11, eCFieldElementHash2FieldElement.multiply(eCPointNormalize.getAffineXCoord())).compareTo(bigInteger) == 0) {
                    return true;
                }
            }
        }
