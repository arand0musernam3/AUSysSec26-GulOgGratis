...
public class ECNRSigner implements DSAExt {
    ...
    private BigInteger extractT(ECPublicKeyParameters eCPublicKeyParameters, BigInteger bigInteger, BigInteger bigInteger2) {
        ...
        return bigInteger.subtract(eCPointNormalize.getAffineXCoord().toBigInteger()).mod(n11);
    }
    ...
    public BigInteger[] generateSignature(byte[] bArr) {
        ...
        if (!this.forSigning) {
            h2.b("not initialised for signing");
            return null;
        }
        ...
        if (bigInteger.compareTo(order) >= 0) {
            i1.t("input too large for ECNR key");
            return null;
        }
        do {
    ...
    public byte[] getRecoveredMessage(BigInteger bigInteger, BigInteger bigInteger2) {
        if (this.forSigning) {
            h2.b("not initialised for verifying/recovery");
            return null;
        }
    ...
    public void init(boolean z11, CipherParameters cipherParameters) {
        ...
        CryptoServicesRegistrar.checkConstraints(Utils.getDefaultProperties("ECNR", this.key, z11));
    }

    @Override // org.bouncycastle.crypto.DSA
    public boolean verifySignature(byte[] bArr, BigInteger bigInteger, BigInteger bigInteger2) {
        if (this.forSigning) {
            h2.b("not initialised for verifying");
            return false;
        }
        ECPublicKeyParameters eCPublicKeyParameters = (ECPublicKeyParameters) this.key;
        BigInteger n11 = eCPublicKeyParameters.getParameters().getN();
        int iBitLength = n11.bitLength();
        ...
        if (bigInteger3.bitLength() <= iBitLength) {
            ...
            return bigIntegerExtractT != null && bigIntegerExtractT.equals(bigInteger3.mod(n11));
        }
        i1.t("input too large for ECNR key.");
        return false;
    }
