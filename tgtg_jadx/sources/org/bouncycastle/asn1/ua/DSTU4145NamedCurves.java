...
public class DSTU4145NamedCurves {
    ...
    static {
        ...
        while (true) {
            ...
            if (i11 >= eCDomainParametersArr.length) {
                break;
            }
            eCDomainParametersArr[i11] = new ECDomainParameters(f2mArr[i11], eCPointArr[i11], bigIntegerArr[i11], bigIntegerArr2[i11]);
            i11++;
        }
        int i12 = 0;
        while (true) {
            ASN1ObjectIdentifier[] aSN1ObjectIdentifierArr = oids;
            if (i12 >= aSN1ObjectIdentifierArr.length) {
                return;
            }
            aSN1ObjectIdentifierArr[i12] = new ASN1ObjectIdentifier(f.h(i12, oidBase, new StringBuilder()));
            i12++;
        }
    }
    ...
    public static ECDomainParameters getByOID(ASN1ObjectIdentifier aSN1ObjectIdentifier) {
        ...
        if (id2.startsWith(oidBase) && (i11 = Integer.parseInt(id2.substring(id2.lastIndexOf(46) + 1))) >= 0) {
            ECDomainParameters[] eCDomainParametersArr = params;
            if (i11 < eCDomainParametersArr.length) {
                return eCDomainParametersArr[i11];
            }
        }
        return null;
    }
