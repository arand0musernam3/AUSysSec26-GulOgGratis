...
public class BouncyCastlePQCProvider extends Provider implements ConfigurableProvider {
    ...
    private static final String[] ALGORITHMS = {"SPHINCS", "LMS", "NH", "XMSS", "SPHINCSPlus", "CMCE", "Frodo", "SABER", "Picnic", "NTRU", "Falcon", "Kyber", "Dilithium", "NTRUPrime", "BIKE", "HQC", "Rainbow", "Mayo", "Snova"};
    ...
    private void loadAlgorithms(String str, String[] strArr) {
        for (int i11 = 0; i11 != strArr.length; i11++) {
            ...
            sbO.append(strArr[i11]);
            sbO.append("$Mappings");
            Class clsLoadClass = loadClass(BouncyCastlePQCProvider.class, sbO.toString());
            if (clsLoadClass != null) {
                try {
                    ((AlgorithmProvider) clsLoadClass.newInstance()).configure(this);
                } catch (Exception e5) {
                    throw new InternalError("cannot create instance of " + str + strArr[i11] + "$Mappings : " + e5);
                }
            }
        }
    }
    ...
    public void addAlgorithm(String str, ASN1ObjectIdentifier aSN1ObjectIdentifier, String str2) {
        if (!containsKey(str + "." + str2)) {
            h2.b(a.d("primary key (", str, ".", str2, ") not found"));
            return;
        }
