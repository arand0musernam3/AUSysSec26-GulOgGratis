...
class WotsPlus {
    ...
    public byte[] pkFromSig(byte[] bArr, byte[] bArr2, byte[] bArr3, ADRS adrs) {
        ...
        base_w(bArr2, 0, this.f33611w, iArr, 0, sLHDSAEngine2.WOTS_LEN1);
        int i11 = 0;
        int i12 = 0;
        while (true) {
            sLHDSAEngine = this.engine;
            if (i11 >= sLHDSAEngine.WOTS_LEN1) {
                break;
            }
            i12 += (this.f33611w - 1) - iArr[i11];
            i11++;
        }
        int i13 = sLHDSAEngine.WOTS_LEN2;
        int i14 = sLHDSAEngine.WOTS_LOGW;
        byte[] bArrIntToBigEndian = Pack.intToBigEndian(i12 << (8 - ((i13 * i14) % 8)));
        int i15 = 4 - (((i13 * i14) + 7) / 8);
        int i16 = this.f33611w;
        SLHDSAEngine sLHDSAEngine3 = this.engine;
        base_w(bArrIntToBigEndian, i15, i16, iArr, sLHDSAEngine3.WOTS_LEN1, sLHDSAEngine3.WOTS_LEN2);
        ...
        while (i17 < this.engine.WOTS_LEN) {
            ...
            byte[] bArr6 = bArr4;
            int i19 = iArr[i17];
            bArr5[i17] = chain(bArr6, i19, (this.f33611w - 1) - i19, bArr3, adrs2);
            i17++;
            adrs2 = adrs;
            bArr4 = bArr6;
        }
    ...
    public byte[] pkGen(byte[] bArr, byte[] bArr2, ADRS adrs) {
        ...
        while (i11 < this.engine.WOTS_LEN) {
            ...
            adrs3.setHashAddress(0);
            byte[] bArr4 = bArr2;
            bArr3[i11] = chain(bArrPRF, 0, this.f33611w - 1, bArr4, adrs3);
            i11++;
            bArr2 = bArr4;
        }
    ...
    public byte[] sign(byte[] bArr, byte[] bArr2, byte[] bArr3, ADRS adrs) {
        ...
        SLHDSAEngine sLHDSAEngine2 = this.engine;
        int[] iArr = new int[sLHDSAEngine2.WOTS_LEN];
        base_w(bArr, 0, this.f33611w, iArr, 0, sLHDSAEngine2.WOTS_LEN1);
        int i11 = 0;
        int i12 = 0;
        while (true) {
            sLHDSAEngine = this.engine;
        ...
        if (i13 % 8 != 0) {
            i12 <<= 8 - ((sLHDSAEngine.WOTS_LEN2 * i13) % 8);
        }
        int i14 = ((sLHDSAEngine.WOTS_LEN2 * i13) + 7) / 8;
