...
public final class XMSSMTSignature implements XMSSStoreableObjectInterface, Encodable {
    ...
    private XMSSMTSignature(Builder builder) {
        ...
        this.reducedSignatures = new ArrayList();
        for (int i11 = iCeil + treeDigestSize; i11 < bArr.length; i11 += height) {
            this.reducedSignatures.add(new XMSSReducedSignature.Builder(this.params.getXMSSParameters()).withReducedSignature(XMSSUtil.extractBytesAtOffset(bArr, i11, height)).build());
        }
    }
    ...
    public byte[] toByteArray() {
        ...
        int i11 = iCeil + treeDigestSize;
        Iterator<XMSSReducedSignature> it = this.reducedSignatures.iterator();
        while (it.hasNext()) {
            XMSSUtil.copyBytesAtOffset(bArr, it.next().toByteArray(), i11);
            i11 += height;
        }
        return bArr;
    }
