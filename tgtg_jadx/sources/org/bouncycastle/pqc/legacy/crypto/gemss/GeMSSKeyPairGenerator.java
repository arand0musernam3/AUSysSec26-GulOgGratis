...
import org.bouncycastle.pqc.legacy.crypto.gemss.GeMSSEngine;
...
public class GeMSSKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
    ...
    public AsymmetricCipherKeyPair generateKeyPair() {
        ...
        int i11 = engine.HFEDegJ + 2;
        int i12 = engine.HFEDegI;
        int i13 = (((i12 + 1) * engine.HFEv) + (engine.NB_MONOMIAL_VINEGAR - 1) + i11 + (((i12 + 1) * i12) >>> 1)) * engine.NB_WORD_GFqn;
        int i14 = (((engine.LTRIANGULAR_NV_SIZE << 1) + i13) + (engine.LTRIANGULAR_N_SIZE << 1)) << 3;
        Pointer pointer = new Pointer(i14 >>> 3);
        byte[] bArr = new byte[i14];
        SHAKEDigest sHAKEDigest = new SHAKEDigest(engine.ShakeBitStrength);
        int i15 = 0;
        sHAKEDigest.update(bArrSec_rand, 0, engine.SIZE_SEED_SK);
        sHAKEDigest.doFinal(bArr, 0, i14);
        int i16 = engine.SIZE_SEED_SK;
        byte[] bArr2 = new byte[i16];
        byte[] bArr3 = new byte[((engine.NB_MONOMIAL_PK * engine.HFEm) + 7) >> 3];
        System.arraycopy(bArrSec_rand, 0, bArr2, 0, i16);
        pointer.fill(0, bArr, 0, i14);
        ...
        Pointer pointer5 = new Pointer(pointer, i13);
        ...
        if (engine.HFEmr8 != 0) {
            int i17 = engine.NB_MONOMIAL_PK;
            int i18 = engine.NB_BYTES_GFqm;
            PointerUnion pointerUnion = new PointerUnion((i17 * i18) + ((8 - (i18 & 7)) & 7));
            for (int i19 = (engine.NB_BYTES_GFqm & 7) != 0 ? 1 : 0; i19 < engine.NB_MONOMIAL_PK; i19++) {
            ...
            if ((engine.NB_BYTES_GFqm & 7) != 0) {
                ...
                while (i15 < engine.NB_WORD_GF2m) {
                    pointerUnion.set(i15, pointer7.get(i15));
                    i15++;
                }
            }
            ...
            int bytesMove = 0;
            while (i15 < engine.NB_MONOMIAL_PK) {
