...
public class Treehash {
    ...
    public Treehash(Digest digest, byte[][] bArr, int[] iArr) {
        ...
        for (int i11 = 0; i11 < this.tailLength; i11++) {
            this.heightOfNodes.addElement(Integers.valueOf(iArr[i11 + 6]));
        }
        this.firstNode = bArr[0];
        this.seedActive = bArr[1];
        this.seedNext = bArr[2];
        this.tailStack = new Vector();
        for (int i12 = 0; i12 < this.tailLength; i12++) {
            this.tailStack.addElement(bArr[i12 + 3]);
        }
    }
    ...
    public int getLowestNodeHeight() {
        ...
        int i11 = this.tailLength;
        int i12 = this.firstNodeHeight;
        return i11 == 0 ? i12 : Math.min(i12, ((Integer) this.heightOfNodes.lastElement()).intValue());
    }
    ...
    public byte[][] getStatByte() {
        ...
        for (int i11 = 0; i11 < this.tailLength; i11++) {
            bArr[i11 + 3] = (byte[]) this.tailStack.elementAt(i11);
        }
        return bArr;
    }
    ...
    public int[] getStatInt() {
        ...
        for (int i12 = 0; i12 < this.tailLength; i12++) {
            iArr[i12 + 6] = ((Integer) this.heightOfNodes.elementAt(i12)).intValue();
        }
        return iArr;
    }
    ...
    public void initialize() {
        if (!this.seedInitialized) {
            h2.b(k.i(this.maxHeight, " not initialized", new StringBuilder("Seed ")));
            return;
        }
        this.heightOfNodes = new Vector();
        this.tailLength = 0;
    ...
    public String toString() {
        String strP = "Treehash    : ";
        for (int i11 = 0; i11 < this.tailLength + 6; i11++) {
            strP = k.i(getStatInt()[i11], " ", i.o(strP));
        }
        for (int i12 = 0; i12 < this.tailLength + 3; i12++) {
            strP = getStatByte()[i12] != null ? k.p(i.o(strP), new String(Hex.encode(getStatByte()[i12])), " ") : strP.concat("null ");
        }
        StringBuilder sbR = f.r(strP, "  ");
        sbR.append(this.messDigestTree.getDigestSize());
        return sbR.toString();
    }
        ...
        if (this.isFinished) {
            ...
            if (this.isInitialized) {
                ...
                if (this.firstNode == null) {
                    ...
                    int i11 = 0;
                    while (this.tailLength > 0 && i11 == ((Integer) this.heightOfNodes.lastElement()).intValue()) {
