...
public abstract class EndoUtil {
    ...
    private static BigInteger calculateB(BigInteger bigInteger, BigInteger bigInteger2, int i11) {
        boolean z11 = bigInteger2.signum() < 0;
        BigInteger bigIntegerMultiply = bigInteger.multiply(bigInteger2.abs());
        boolean zTestBit = bigIntegerMultiply.testBit(i11 - 1);
        BigInteger bigIntegerShiftRight = bigIntegerMultiply.shiftRight(i11);
        if (zTestBit) {
            bigIntegerShiftRight = bigIntegerShiftRight.add(ECConstants.ONE);
        }
        return z11 ? bigIntegerShiftRight.negate() : bigIntegerShiftRight;
    }
