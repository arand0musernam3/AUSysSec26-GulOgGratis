...
public abstract class BaseAgreementSpi extends KeyAgreementSpi {
    ...
    private byte[] calcSecret() {
        ...
        boolean zIsPrependedT = this.hybridSpec.isPrependedT();
        HybridValueParameterSpec hybridValueParameterSpec = this.hybridSpec;
        byte[] bArrConcatenate = zIsPrependedT ? Arrays.concatenate(hybridValueParameterSpec.getT(), bArrDoCalcSecret) : Arrays.concatenate(bArrDoCalcSecret, hybridValueParameterSpec.getT());
    ...
    private byte[] getSharedSecretBytes(byte[] bArr, String str, int i11) throws NoSuchAlgorithmException {
        ...
        if (i11 < 0) {
            throw new NoSuchAlgorithmException(f.k("unknown algorithm encountered: ", str));
        }
        int i13 = i11 / 8;
        ...
        if (derivationFunction instanceof DHKEKGenerator) {
            ...
            try {
                kDFParameters = new DHKDFParameters(new ASN1ObjectIdentifier(str), i11, bArr, this.ukmParameters);
    ...
    public int engineGenerateSecret(byte[] bArr, int i11) throws IllegalStateException, ShortBufferException {
        ...
        if (bArr.length - i11 >= bArrEngineGenerateSecret.length) {
            System.arraycopy(bArrEngineGenerateSecret, 0, bArr, i11, bArrEngineGenerateSecret.length);
            return bArrEngineGenerateSecret.length;
        }
        StringBuilder sb2 = new StringBuilder();
        sb2.append(this.kaAlgorithm);
        sb2.append(" key agreement: need ");
        throw new ShortBufferException(k.i(bArrEngineGenerateSecret.length, " bytes", sb2));
    }
