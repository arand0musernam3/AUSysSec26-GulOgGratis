...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException {
        if (!X509SignatureUtil.areEquivalentAlgorithms(this.f33300c.getSignatureAlgorithm(), this.f33300c.getTBSCertificate().getSignature())) {
        ...
        try {
            ...
            this.f33300c.getTBSCertificate().encodeTo(bufferedOutputStream, ASN1Encoding.DER);
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws NoSuchAlgorithmException, SignatureException, InvalidKeyException, CertificateException, NoSuchProviderException {
        ...
        int i11 = 0;
        if (z11 && X509SignatureUtil.isCompositeAlgorithm(this.f33300c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence aSN1Sequence = ASN1Sequence.getInstance(this.f33300c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence2 = ASN1Sequence.getInstance(this.f33300c.getSignature().getOctets());
            boolean z12 = false;
        ...
        if (!X509SignatureUtil.isCompositeAlgorithm(this.f33300c.getSignatureAlgorithm())) {
            ...
            if (z11) {
                ...
                if (MiscObjectIdentifiers.id_composite_key.equals((ASN1Primitive) compositePublicKey.getAlgorithmIdentifier())) {
                    ...
                    while (i11 != publicKeys2.size()) {
                        try {
                            checkSignature(publicKeys2.get(i11), signatureCreateSignature, this.f33300c.getSignatureAlgorithm().getParameters(), getSignature());
                            return;
                        } catch (InvalidKeyException unused) {
                            i11++;
                        }
                    }
                    a.i("no matching signature found");
                    return;
                }
            }
            checkSignature(publicKey, signatureCreateSignature, this.f33300c.getSignatureAlgorithm().getParameters(), getSignature());
            return;
        }
        ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(this.f33300c.getSignatureAlgorithm().getParameters());
        ASN1Sequence aSN1Sequence4 = ASN1Sequence.getInstance(this.f33300c.getSignature().getOctets());
        boolean z13 = false;
    ...
    public void checkValidity(Date date) throws CertificateNotYetValidException, CertificateExpiredException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f33300c.getEndDate().getTime());
        }
        if (date.getTime() >= getNotBefore().getTime()) {
            return;
        }
        throw new CertificateNotYetValidException("certificate not valid till " + this.f33300c.getStartDate().getTime());
    }
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f33300c.getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extensionOctets = getExtensionOctets(this.f33300c, Extension.extendedKeyUsage);
        if (extensionOctets == null) {
            return null;
        }
        try {
    ...
    public byte[] getExtensionValue(String str) {
        return X509SignatureUtil.getExtensionValue(this.f33300c.getExtensions(), str);
    }

    @Override // java.security.cert.X509Certificate
    public Collection getIssuerAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f33300c, Extension.issuerAlternativeName);
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f33300c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getIssuerUniqueID() {
        ASN1BitString issuerUniqueId = this.f33300c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Name getIssuerX500Name() {
        return this.f33300c.getIssuer();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f33300c.getIssuer().getEncoded(ASN1Encoding.DER));
    ...
    public Date getNotAfter() {
        return this.f33300c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotBefore() {
        return this.f33300c.getStartDate().getDate();
    }

    @Override // java.security.cert.Certificate
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f33300c.getSubjectPublicKeyInfo());
        } catch (IOException e5) {
            throw Exceptions.illegalStateException("failed to recover public key: " + e5.getMessage(), e5);
        }
    }

    @Override // java.security.cert.X509Certificate
    public BigInteger getSerialNumber() {
        return this.f33300c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f33300c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f33300c.getSignature().getOctets();
    }

    @Override // java.security.cert.X509Certificate
    public Collection getSubjectAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f33300c, Extension.subjectAlternativeName);
    }

    @Override // java.security.cert.X509Certificate
    public Principal getSubjectDN() {
        return new X509Principal(this.f33300c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getSubjectUniqueID() {
        ASN1BitString subjectUniqueId = this.f33300c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Name getSubjectX500Name() {
        return this.f33300c.getSubject();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f33300c.getSubject().getEncoded(ASN1Encoding.DER));
        } catch (IOException unused) {
            h2.b("can't encode subject DN");
            return null;
        }
    }
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f33300c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e5) {
            throw new CertificateEncodingException(e5.toString());
        }
    }
    ...
    public TBSCertificate getTBSCertificateNative() {
        return this.f33300c.getTBSCertificate();
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f33300c.getVersionNumber();
    }

    @Override // java.security.cert.X509Extension
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f33300c.getExtensions()) == null) {
            return false;
        }
        Enumeration enumerationOids = extensions.oids();
        while (enumerationOids.hasMoreElements()) {
