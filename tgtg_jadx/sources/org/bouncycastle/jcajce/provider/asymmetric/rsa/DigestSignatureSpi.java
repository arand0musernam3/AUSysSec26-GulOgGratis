...
public class DigestSignatureSpi extends SignatureSpi {
    ...
    public void engineInitSign(PrivateKey privateKey) throws InvalidKeyException {
        if (!(privateKey instanceof RSAPrivateKey)) {
            throw new InvalidKeyException(k.p(new StringBuilder("Supplied key ("), getType(privateKey), ") is not a RSAPrivateKey instance"));
        }
    ...
    public void engineInitVerify(PublicKey publicKey) throws InvalidKeyException {
        if (!(publicKey instanceof RSAPublicKey)) {
            throw new InvalidKeyException(k.p(new StringBuilder("Supplied key ("), getType(publicKey), ") is not a RSAPublicKey instance"));
        }
    ...
    public boolean engineVerify(byte[] bArr) throws SignatureException {
        ...
        byte b8 = (byte) (bArrDerEncode[3] - 2);
        bArrDerEncode[3] = b8;
        int i11 = b8 + 4;
        int i12 = b8 + 6;
        int i13 = 0;
        for (int i14 = 0; i14 < bArrDerEncode.length - i12; i14++) {
            i13 |= bArrProcessBlock[i11 + i14] ^ bArrDerEncode[i12 + i14];
        }
        for (int i15 = 0; i15 < i11; i15++) {
            i13 |= bArrProcessBlock[i15] ^ bArrDerEncode[i15];
        }
        return i13 == 0;
    }
