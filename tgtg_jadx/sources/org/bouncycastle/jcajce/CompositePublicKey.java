...
public class CompositePublicKey implements PublicKey {
    ...
    public byte[] getEncoded() {
        ...
        for (int i11 = 0; i11 < this.keys.size(); i11++) {
            boolean zEquals = this.algorithmIdentifier.equals((ASN1Primitive) MiscObjectIdentifiers.id_composite_key);
            List<PublicKey> list = this.keys;
            aSN1EncodableVector.add(zEquals ? SubjectPublicKeyInfo.getInstance(list.get(i11).getEncoded()) : SubjectPublicKeyInfo.getInstance(list.get(i11).getEncoded()).getPublicKeyData());
        }
        try {
            ...
            h2.b(s.g(e5, new StringBuilder("unable to encode composite public key: ")));
            return null;
        }
    }
    ...
    public CompositePublicKey(ASN1ObjectIdentifier aSN1ObjectIdentifier, PublicKey... publicKeyArr) {
        ...
        if (publicKeyArr == null || publicKeyArr.length == 0) {
            a.f("at least one public key must be provided for the composite public key");
            throw null;
        }
